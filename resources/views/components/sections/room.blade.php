    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-8">
            <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div class="flex flex-wrap gap-2">
                    <button 
                        class="filter-btn active px-4 py-2 rounded-lg font-medium transition-all duration-200"
                        data-filter="all"
                    >
                        Semua Ruangan
                    </button>
        ...
        <div class="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3" id="rooms-container">
            @foreach ($ruangans as $ruangan)
                <div class="room-card bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden border border-gray-200" 
                        ...
                        <div class="absolute top-3 right-3">
                            @if($ruangan->is_currently_used)
                                @if($ruangan->current_booking)
                                    <span class="px-3 py-1 bg-red-500 text-white text-xs font-medium rounded-full shadow-lg">
                                        Sedang Digunakan
                                    </span>
                                @else
                    ...
                    <div class="p-6">
                            ...
                            @if($ruangan->current_booking)
                                ...
                                <div class="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                                    ...
                                    <div class="text-xs text-red-700 space-y-1">
                                        ...
                                        <p><strong>Keperluan:</strong> {{ Str::limit($ruangan->current_booking->keperluan, 30) }}</p>
                                    ...
                                    @foreach($ruangan->upcoming_bookings as $booking)
                                        <div class="text-xs bg-gray-50 p-2 rounded border">
                                            <div class="flex justify-between items-start">
                                                <span class="font-medium text-gray-700">{{ \Carbon\Carbon::parse($booking->tanggal_pinjam)->format('d/m') }}</span>
                                            ...
                                            <p class="text-gray-600 truncate">{{ $booking->waktu_mulai }}-{{ $booking->waktu_selesai }} | {{ $booking->pengguna->nama ?? 'N/A' }}</p>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        @auth
                            @if(!auth()->user()->hasRole('admin'))
                                <a href="{{ route('ruangan.show', $ruangan) }}" class="block">
                                    <button class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-lg transition-all duration-200 flex items-center justify-center">
                            ...
                            @else
                                <div class="w-full bg-gray-400 text-white font-medium py-3 px-4 rounded-lg flex items-center justify-center cursor-not-allowed">
                        ...
                        @else
                            <a href="{{ route('login') }}" class="block">
                                <button class="w-full bg-gray-500 hover:bg-gray-600 text-white font-medium py-3 px-4 rounded-lg transition-all duration-200 flex items-center justify-center">
        ...
        <div id="no-results" class="hidden text-center py-12">
            <div class="mx-auto w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                <svg class="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.287 0-4.348-.773-6-2.062M15 21H3a2 2 0 01-2-2V5a2 2 0 012-2h12a2 2 0 012 2v14a2 2 0 01-2 2z"></path>
                </svg>
            </div>
