...
class RoomController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function($row){
                    $editBtn = '<button onclick="loadEditModal(\'ruangan\', '.$row->id_ruang.')" type="button" id="edit-ruangan-'.$row->id_ruang.'" class="btn-edit bg-yellow-500 hover:bg-yellow-600 text-white p-2 rounded transition duration-150 mr-1">
                    ...
                    $deleteBtn = '<form method="POST" action="'.route('ruangan.destroy', $row->id_ruang).'" style="display: inline;">
                        '.csrf_field().'
                        '.method_field('DELETE').'
                        <button type="submit" class="btn-delete bg-red-500 hover:bg-red-600 text-white p-2 rounded transition duration-150" data-item-name="ruangan '.$row->nama_ruangan.'">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    ...
    public function destroy($id)
    {
        ...
        // Delete related foreign key records first
        $ruangan->jadwal()->delete(); // If there's a schedule relationship
        $ruangan->peminjaman()->delete(); // If there's a booking/borrowing relationship
